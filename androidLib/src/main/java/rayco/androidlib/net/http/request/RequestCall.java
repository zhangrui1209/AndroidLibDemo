...
public class RequestCall {
    private HttpRequest okHttpRequest;
    ...
    public Call buildCall(Callback callback) {
        ...
        if (readTimeOut > 0 || writeTimeOut > 0 || connTimeOut > 0) {
            readTimeOut = readTimeOut > 0 ? readTimeOut : Http.DEFAULT_MILLISECONDS;
            writeTimeOut = writeTimeOut > 0 ? writeTimeOut : Http.DEFAULT_MILLISECONDS;
            connTimeOut = connTimeOut > 0 ? connTimeOut : Http.DEFAULT_MILLISECONDS;

            clone = Http.getInstance().getOkHttpClient().newBuilder()
            ...
            call = Http.getInstance().getOkHttpClient().newCall(request);
        }
        return call;
    }
